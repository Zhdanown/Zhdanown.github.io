...
Touch_Singleton.prototype.inside = function (object) {
	var xDist = object.position.x - Touch.getPosition(0).x,
		yDist = object.position.y - Touch.getPosition(0).y;
	if (Math.abs(xDist) < object.size.x / 2 &&
		Math.abs(yDist) < object.size.y / 2 ) {
		return true;
	} else
		return false;
};
...
Touch_Singleton.prototype.getTouchIndexFromId = function (id) {
	for (var i = 0, l = this._touches.length; i < length; ++i) {
